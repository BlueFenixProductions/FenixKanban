...
### #73 — Per-board Fizzy Sync in the board menu (#18 follow-on) (2026-06-26)

Surfaced per-board Fizzy sync where the user actually is: the board `···` menu. New
`BoardFizzySyncModel` (@MainActor @Observable) derives a 3-state projection
(`notConfigured`/`unpaired`/`paired`) synchronously from the auth state + pairing store and
delegates every action to `FizzySyncProvider` — no sync logic duplicated. RED 1/GREEN 1: pure
`derive(isConfigured:pairing:)` (5 tests). RED 2/GREEN 2: 6 action wirings (toggleSync, syncNow,
unpair, createOnFizzy, linkExisting, loadUnpairedRemoteBoards) against the real-provider Harness
with stubbed HTTP (13 tests). Refactor: extracted `LinkBoardSheet` from FizzyBoardBrowserView into
its own internal file, shared by the browser and the new menu. BoardView gains a state-adaptive
"Fizzy Sync" menu section + setup/link sheets, unpair confirmation, and error/collision alerts
(in a `FizzySyncPresentations` modifier). iOS suite green, macOS build green, no warnings.
GREEN 2 method bodies were generated by the local LLM on Hinata (qwen2.5-coder-14b-instruct) from
the design-doc delegation table, gated through the elf-dispatch repo policy; commit subjects drafted
locally too.
